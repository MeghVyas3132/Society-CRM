...
class User {
  ...
  /// Helper function to parse dates (handles both ISO strings and Firestore Timestamps)
    ...
    try {
      ...
      if (dateValue is Map) {
        ...
        if (dateValue.containsKey('_seconds')) {
          ...
          if (seconds is num) {
            return DateTime.fromMillisecondsSinceEpoch((seconds.toInt() * 1000).toInt());
          }
        }
        // Alternative format: milliseconds directly
        if (dateValue.containsKey('milliseconds')) {
          final ms = dateValue['milliseconds'];
          if (ms is num) {
      ...
      // If it's a number (milliseconds since epoch or seconds)
      if (dateValue is num) {
        // If it's too large (more than year 3000 in milliseconds), treat as milliseconds
        // Otherwise treat as seconds
        if (dateValue > 32503680000) {
          return DateTime.fromMillisecondsSinceEpoch(dateValue.toInt());
        } else {
          return DateTime.fromMillisecondsSinceEpoch((dateValue.toInt() * 1000).toInt());
        }
      }
